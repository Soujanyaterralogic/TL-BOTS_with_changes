...
class Actions(TelnetHandler, object):
    ...
    def generic_logs5(self, cmd, data, cmd_config):
        try:
            config = NECFG[cmd["TID"]].get("MODIFIER", {})
            attributes = config.get("attributes", {})
            total_shelves = int(cmd_config.get('no_of_shelf', 1))
            ...
            def fetch_attribute_value(card_name,attribute_name):
                if attribute_name == "OVERTEMP":
                    ...
                    elif card_name == "PEP5-PEA2":
                        return "VFSA"
                    else:
                        return random.choice(["SWFO", "SAS"])
                elif attribute_name == "SMODE":
                    if card_name == "PEP5-PEA2":
                        return "FLOW"
                    else:
                        return random.choice(["MODE1", "MODE2"])
                elif attribute_name == "CARDCURRFEED":
                    return "{:.1f}A".format(random.uniform(0.1, 70.0))
                return "Unknown"
            ...
            for shelf_number in range(1, total_shelves + 1):
                ...
                for card_name in card_names:
                    ...
                    if card_name == "IFP5-CTC1":
                        ...
                        status = "OOS-AU,SGEO&ACT"
                        card_log = '"{aid}:{card_name}:ACTTYPE={card_name},VENDID={vend_id},DOM={dom},CLEI={clei},SERIALNO={serial_no},USI={usi},MODE={MODE},SMODE={SMODE},MAID={MAID},CARDCURRFEED=6.5A:{status}"'.format(
                            aid=aid, card_name=card_info['cardName'], vend_id=vend_id, dom=dom, clei=clei,
                            serial_no=serial_no, usi=usi, status=status,
                            MODE=fetch_attribute_value(card_name, 'MODE'),
                            SMODE=fetch_attribute_value(card_name, 'SMODE'),
                            MAID=fetch_attribute_value(card_name, 'MAID')
                        )
                    else:
                        extra_fields = ""
                        for attr in card_attributes.get(card_name, []):
    ...
    def handel_generic_eqpt(self, cmd, data, cmd_config):
        file_path = '/home/vboxuser/Downloads/tl1/products/fujitsu/flashwave_9500/thale.json'

        with open(file_path, 'r') as f:
            user_eqpt_data = json.load(f)
        try:
            tid_config = user_eqpt_data["TID"]
            card_attributes = user_eqpt_data["TID"]["card_attributes"]
            aids_config = tid_config.get("aids", {})
            attributes = tid_config.get("attributes", {})
            total_shelves = int(cmd_config.get('no_of_shelf', 1))
            ...
            for shelf_number in range(1, total_shelves + 1):
                shelf_card_entries = tid_config['aids']['shelfCards'].get('shelf{}'.format(shelf_number), [])
                for shelf_card in shelf_card_entries:
                    vend_id = random_vendid()
                    dom = datetime.now().strftime("%y.%m")
                    clei = random_value(attributes['cleiPattern'])
                ...
                for card_name in card_names:
                    ...
                    if slot_index > slots_to_fill:
                        break
                    card_info = next((card for card in tid_config['aids']['cards'] if card['cardName'] == card_name), None)
                    if not card_info:
                        continue
                ...
                for special_type in tid_config['aids']['specialTypes'].get('shelf{}'.format(shelf_number), []):
                    aid = special_type['aid']
                    card_name = special_type['cardName']
                    vend_id = random_vendid()
                    dom = datetime.now().strftime("%y.%m")
    ...
    def random_value(self, pattern):
        from string import digits, ascii_uppercase
        chars = digits + ascii_uppercase
        return ''.join(random.choice(chars) for _ in range(int(pattern.split('{')[1].split('}')[0])))

    def random_vendid(self):
        return "FC" + "".join(str(random.randint(0, 9)) for _ in range(6)) + "-" + "".join(
            str(random.randint(0, 9)) for _ in range(2))

    def generate_overtemp(self):
        return str(random.randint(0, 150))

    def generate_card_current_feed(self):
        current_feed = random.uniform(0.1, 70.0)
        return "{:.1f}A".format(current_feed)

    def generate_current_draw(self, pattern):
        if 'xx.xA' in pattern:
            return "{:.1f}A".format(random.uniform(0.1, 15.0))
        return pattern
    ...
    def fetch_attribute_value(self, card_name, attribute_name):
        if attribute_name == "OVERTEMP":
            return str(random.randint(0, 150))
        elif attribute_name in ["FREQMIN", "FREQMAX"]:
            return "{:.5f}".format(random.uniform(191.5, 196.5))
    ...
    def generate_shelf_card_log(self, shelf_card, attributes):
        vend_id = self.random_vendid()
        dom = datetime.now().strftime("%y.%m")
        clei = self.random_value(attributes['cleiPattern'])
        serial_no = self.generate_serial_number()
        ...
        return '"{aid}:{card_name}:ACTTYPE={card_name},VENDID={vend_id},DOM={dom},CLEI={clei},SERIALNO={serial_no},USI={usi},VOLTAGE={voltage},FUSEFEED1={fuse_feed1},FUSEFEED2={fuse_feed2},CURRENTDRAWNFEED={current_draw},CURRENTDRAWNFEED1={current_draw1},CURRENTDRAWNFEED2={current_draw2}:IS-NR,ACT"'.format(
                        aid=shelf_card['aid'], card_name=shelf_card['cardName'], vend_id=vend_id, dom=dom, clei=clei,
                        serial_no=serial_no, usi=usi, voltage=voltage, current_draw=current_draw,
                        current_draw1=current_draw1, current_draw2=current_draw2, fuse_feed1=fuse_feed1,
                        fuse_feed2=fuse_feed2)

    def generate_card_log(self, aid, card_info, attributes, card_name, fetch_attribute_value, card_attributes):
        vend_id = self.random_vendid()
        dom = datetime.now().strftime("%y.%m")
        clei = self.random_value(attributes['cleiPattern'])
        serial_no = self.generate_serial_number()
        usi = self.random_value(attributes['usiPattern'])
        overtemp = self.generate_overtemp()
        maid = random.choice(attributes['MAID'])

        if card_name == "IFP5-CTC1":
            status = "OOS-AU,UEQ&ACT"
            return '"{aid}:{card_name}:ACTTYPE=,VENDID=,DOM=,CLEI=,SERIALNO=,USI=,MODE=SWFO,OVERTEMP=-99.9,CARDCURRFEED=3.1A:{status}"'.format(
                aid=aid, card_name=card_info['cardName'], vend_id=vend_id, dom=dom, clei=clei,
                serial_no=serial_no, usi=usi, status=status)
        elif card_name == "PEP5-PEA2":
            status = "OOS-AU,SGEO&ACT"
            return '"{aid}:{card_name}:ACTTYPE={card_name},VENDID={vend_id},DOM={dom},CLEI={clei},SERIALNO={serial_no},USI={usi},MODE={MODE},SMODE={SMODE},MAID={MAID},CARDCURRFEED=6.5A:{status}"'.format(
                aid=aid, card_name=card_info['cardName'], vend_id=vend_id, dom=dom, clei=clei,
                serial_no=serial_no, usi=usi, status=status,
                MODE=fetch_attribute_value(card_name, 'MODE'),
                SMODE=fetch_attribute_value(card_name, 'SMODE'),
            ...
            return '"{aid}:{card_name}:ACTTYPE={card_name},VENDID={vend_id},DOM={dom},CLEI={clei},SERIALNO={serial_no},USI={usi}{extra}:IS-NR,ACT"'.format(
                aid=aid, card_name=card_info['cardName'], vend_id=vend_id, dom=dom, clei=clei,
                serial_no=serial_no, usi=usi,
                extra=extra_fields)

    def generate_port_log(self, card_name, card_info, port_index, aid, attributes):
        serial_no = self.generate_serial_number()
        dom = datetime.now().strftime("%y.%m")
        clei = self.random_value(attributes['cleiPattern'])
        usi = self.random_value(attributes['usiPattern'])
        ...
        if card_name == "IFP5-CMD1":
            if 1 <= port_index <= 4:
                status = "IS-NR,ACT"
                return '"{port_aid}:{port_type}:ACTTYPE={port_type},TYPEINFO01={port_type},COMPLTYPE={port_type},VENDID={vend_id},DOM={dom},CLEI={clei},SERIALNO={serial_no},USI={usi},LAMBDA=UNSPEC,LAMBDAINFO=1310:{status}"'.format(
                    port_aid=port_aid, port_type=port_type, vend_id=vend_id, dom=dom, clei=clei,
                    serial_no=serial_no, usi=usi, status=status)
            elif 5 <= port_index <= 8:
                status = "OOS-AU,UEQ&ACT"
                return '"{port_aid}:{port_type}:ACTTYPE=,TYPEINFO01=,COMPLTYPE={port_type},VENDID=,DOM=,CLEI=,SERIALNO=,USI=,LAMBDA=UNSPEC,LAMBDAINFO=:{status}"'.format(
                    port_aid=port_aid, port_type=port_type, status=status)

        if card_name == "IFP5-TGD1":
            if port_index == 1:
                status = "IS-NR,ACT"
                return '"{port_aid}::ACTTYPE=OC48IR1&OCH27IR1,TYPEINFO01=OC48IR1&OC12IR1&OC3IR1&1GELX10&100LX10&OCH27IR1,COMPLTYPE=,VENDID={vend_id},DOM={dom},CLEI={clei},SERIALNO={serial_no},USI={usi},LAMBDA=,LAMBDAINFO=UNKNOWN:{status}"'.format(
                    port_aid=port_aid, vend_id=vend_id, dom=dom, clei=clei, serial_no=serial_no,
                    usi=usi,
                    status=status)
            else:
                status = "OOS-AUMA,UAS&UEQ"
                return '"{port_aid}::ACTTYPE=,TYPEINFO01=,COMPLTYPE=,VENDID=,DOM=,CLEI=,SERIALNO=,USI=,LAMBDA=,LAMBDAINFO=:{status}"'.format(
                    port_aid=port_aid, status=status)

        if card_name == "IFP5-CXF4":
            if port_index == 1:
                status = "OOS-MA,UAS"
                return '"{port_aid}::ACTTYPE=OC48SR1&OCH27SR1,TYPEINFO01=OC48SR1&OC12SR1&OC3SR1&1GELX&OCH27SR1,COMPLTYPE=,VENDID={vend_id},DOM={dom},CLEI={clei},SERIALNO={serial_no},USI={usi},LAMBDA=,LAMBDAINFO=UNKNOWN:{status}"'.format(
                    port_aid=port_aid, vend_id=vend_id, dom=dom, clei=clei, serial_no=serial_no,
                    usi=usi,
                    status=status)
            elif 2 <= port_index <= 14:
                if port_index == 13:
                    status = "OOS-MA,UAS"
                    return '"{port_aid}::ACTTYPE=,TYPEINFO01=,COMPLTYPE=,VENDID={vend_id},DOM={dom},CLEI={clei},SERIALNO={serial_no},USI={usi},LAMBDA=,LAMBDAINFO=UNKNOWN:{status}"'.format(
                        port_aid=port_aid, vend_id=vend_id, dom=dom, clei=clei, serial_no=serial_no,
                        usi=usi, status=status)
                else:
                    status = "OOS-AUMA,UAS&UEQ"
        ...
        if card_name == "IFP5-TCA2":
            if port_index == 1:
                status = "OOS-MA,UAS"
                return '"{port_aid}::ACTTYPE=OCH430LR4&40GELR4,TYPEINFO01=40GELR4&OCH430LR4,COMPLTYPE=,VENDID={vend_id},DOM={dom},CLEI={clei},SERIALNO={serial_no},USI={usi},LAMBDA=,LAMBDAINFO=UNKNOWN:{status}"'.format(
                    port_aid=port_aid, vend_id=vend_id, dom=dom, clei=clei, serial_no=serial_no,
                    usi=usi,
                    status=status)

        if card_name == "IFP5-S9B1":
            if port_index == 1:
                status = "IS-NR,ACT"
                return '"{port_aid}:{port_type}:ACTTYPE={port_type},TYPEINFO01={port_type},COMPLTYPE={port_type},VENDID={vend_id},DOM={dom},CLEI={clei},SERIALNO={serial_no},USI={usi},LAMBDA=UNSPEC,LAMBDAINFO=1310:{status}"'.format(
                    port_aid=port_aid, port_type=port_type, vend_id=vend_id, dom=dom, clei=clei,
        ...
        if card_name == "IFP5-EGS1":
            status = "OOS-AU,UEQ&ACT"
            return '"{port_aid}:{port_type}:ACTTYPE=,TYPEINFO01=,COMPLTYPE={port_type},VENDID=,DOM=,CLEI=,SERIALNO=,USI=,LAMBDA=UNSPEC,LAMBDAINFO=:{status}"'.format(
                port_aid=port_aid, port_type=port_type, status=status)

        if card_name == "IFP5-CMS1":
            if 1 <= port_index <= 3:
                status = "IS-NR,ACT"
                return '"{port_aid}:{port_type}:ACTTYPE={port_type},TYPEINFO01={port_type},COMPLTYPE={port_type},VENDID={vend_id},DOM={dom},CLEI={clei},SERIALNO={serial_no},USI={usi},LAMBDA=UNSPEC,LAMBDAINFO=850:{status}"'.format(
                    port_aid=port_aid, port_type=port_type, vend_id=vend_id, dom=dom, clei=clei,
                    serial_no=serial_no, usi=usi, status=status)
            elif port_index == 5:
                status = "OOS-AU,MEA&ACT"
                ...
                status = "OOS-AU,MEA&ACT"
                return '"{port_aid}:{port_type}:ACTTYPE={port_type},TYPEINFO01={port_type},COMPLTYPE={port_type},VENDID={vend_id},DOM={dom},CLEI={clei},SERIALNO={serial_no},USI={usi},LAMBDA=UNSPEC,LAMBDAINFO=1550:{status}"'.format(
                    port_aid=port_aid, port_type=port_type, vend_id=vend_id, dom=dom, clei=clei,
                    serial_no=serial_no, usi=usi, status=status)
            elif port_index in [4, 6, 8, 9, 10] + list(range(12, 21)):
                status = "OOS-AU,UEQ&ACT"
                ...
                status = "OOS-AU,MEA&ACT"
                return '"{port_aid}:{port_type}:ACTTYPE=,TYPEINFO01=,COMPLTYPE={port_type},VENDID={vend_id},DOM={dom},CLEI={clei},SERIALNO={serial_no},USI={usi},LAMBDA=UNSPEC,LAMBDAINFO=1310:{status}"'.format(
                    port_aid=port_aid, port_type=port_type, vend_id=vend_id, dom=dom, clei=clei,
                    serial_no=serial_no, usi=usi, status=status)

        if card_name == "IFP5-TMD1":
            if 1 <= port_index <= 8:
                status = "OOS-AUMA,UAS&UEQ"
                return '"{port_aid}::ACTTYPE=,TYPEINFO01=,COMPLTYPE=,VENDID=,DOM=,CLEI=,SERIALNO=,USI=,LAMBDA=,LAMBDAINFO=:{status}"'.format(
                    port_aid=port_aid, status=status)
        ...
        if card_name == "IFP5-CTC1":
            if 1 <= port_index <= 4:
                status = "OOS-MA,UAS"
                return '"{port_aid}::ACTTYPE=,TYPEINFO01=,COMPLTYPE=,VENDID=,DOM=,CLEI=,SERIALNO=,USI=,LAMBDA=:{status}"'.format(
                    port_aid=port_aid, status=status)

        if card_name == "IFP5-EXX1":
            if port_index == 1:
                status = "IS-NR,ACT"
                return '"{port_aid}:{port_type}:ACTTYPE={port_type},TYPEINFO01={port_type},COMPLTYPE={port_type},VENDID={vend_id},DOM={dom},CLEI={clei},SERIALNO={serial_no},USI={usi},LAMBDA=UNSPEC,LAMBDAINFO=1310:{status}"'.format(
                    port_aid=port_aid, port_type=port_type, vend_id=vend_id, dom=dom, clei=clei,
    ...
    def generate_special_log(self, special_type, attributes):
        aid = special_type['aid']
        card_name = special_type['cardName']
        vend_id = self.random_vendid()
        dom = datetime.now().strftime("%y.%m")
        ...
        usi = self.random_value(attributes['usiPattern'])
        overtemp = self.generate_overtemp()
        card_curr_feed = self.generate_card_current_feed()

        if card_name == "MPP5-MPE2" and aid.endswith("1"):
            status = "IS-NR,ACT"
            return '"{aid}:{card_name}:ACTTYPE={card_name},VENDID={vend_id},DOM={dom},CLEI={clei},SERIALNO={serial_no},USI={usi},OVERTEMP={overtemp},CARDCURRFEED={card_curr_feed}:{status}"'.format(
                aid=aid, card_name=card_name, vend_id=vend_id, dom=dom, clei=clei, serial_no=serial_no,
                usi=usi, overtemp=overtemp, card_curr_feed=card_curr_feed, status=status)
        elif card_name == "MPP5-MPE2" and aid.endswith("2"):
            status = "OOS-AU,UEQ&STBYH"
            return '"{aid}:{card_name}:ACTTYPE=,VENDID=,DOM=,CLEI=,SERIALNO=,USI=,OVERTEMP=-99.9,CARDCURRFEED={card_curr_feed}:{status}"'.format(
                aid=aid, card_name=card_name, card_curr_feed=card_curr_feed, status=status)

        if card_name == "SWP5-SF52" and aid.endswith("1"):
            status = "IS-NR,ACT"
            return '"{aid}:{card_name}:ACTTYPE={card_name},VENDID={vend_id},DOM={dom},CLEI={clei},SERIALNO={serial_no},USI={usi},OVERTEMP={overtemp},CAPACITY_ILF=NONE,CARDCURRFEED={card_curr_feed},TDM_INUSE=160,PKT_INUSE=50,OTN_INUSE=65:{status}"'.format(
                aid=aid, card_name=card_name, vend_id=vend_id, dom=dom, clei=clei, serial_no=serial_no,
                usi=usi, overtemp=overtemp, card_curr_feed=card_curr_feed, status=status)
        elif card_name == "SWP5-SF52" and aid.endswith("2"):
            status = "OOS-AU,UEQ&STBYH"
            return '"{aid}:{card_name}:ACTTYPE=,VENDID=,DOM=,CLEI=,SERIALNO=,USI=,OVERTEMP=-99.9,CAPACITY_ILF=NONE,CARDCURRFEED={card_curr_feed},TDM_INUSE=160,PKT_INUSE=50,OTN_INUSE=65:{status}"'.format(
                aid=aid, card_name=card_name, card_curr_feed=card_curr_feed, status=status)

        return '"{aid}:{card_name}:ACTTYPE={card_name},VENDID={vend_id},DOM={dom},CLEI={clei},SERIALNO={serial_no},USI={usi},CARDCURRFEED={card_curr_feed}:IS-NR,ACT"'.format(
            aid=aid, card_name=card_name, vend_id=vend_id, dom=dom, clei=clei, serial_no=serial_no,
            usi=usi, card_curr_feed=card_curr_feed)

    def handel_generic_eqpt1(self, cmd, data, cmd_config):
        file_path = '/home/vboxuser/Downloads/tl1/products/fujitsu/flashwave_9500/thale.json'
        ...
        try:
            tid_config = user_eqpt_data["TID"]
            card_attributes = tid_config["card_attributes"]
            aids_config = tid_config.get("aids", {})
            attributes = tid_config.get("attributes", {})
            total_shelves = int(cmd_config.get('no_of_shelf', 1))
            ...
            for shelf_number in range(1, total_shelves + 1):
                shelf_card_entries = aids_config['shelfCards'].get('shelf{}'.format(shelf_number), [])
                for shelf_card in shelf_card_entries:
                    shelf_card_log = self.generate_shelf_card_log(shelf_card, attributes)
                    logs.append(shelf_card_log)
                ...
                for card_name in card_names:
                    ...
                    if slot_index > slots_to_fill:
                        break
                    card_info = next(
                        (card for card in aids_config['cards'] if card['cardName'] == card_name), None)
                    if not card_info:
                        continue

                    aid = "{}-{}".format(shelf_number, slot_index)
                    card_log = self.generate_card_log(aid, card_info, attributes, card_name, self.fetch_attribute_value,
                                                      card_attributes)
                    logs.append(card_log)

                    for port_index in range(1, card_info['ports'] + 1):
                        port_log = self.generate_port_log(card_name, card_info, port_index, aid, attributes)
                        logs.append(port_log)
                    slot_index += 1
                ...
                for special_type in aids_config['specialTypes'].get('shelf{}'.format(shelf_number), []):
                    special_log = self.generate_special_log(special_type, attributes)
                    logs.append(special_log)
    ...
    def handle_init_sys1(self, cmd, data, cmd_config):
        print("before update", data)
        tid = cmd["TID"]
        params = dict(param.split('=') for param in cmd["KWBLOCK"].split(','))

        # Ensure data structure for VERSION is initialized with default values
        if tid not in data:
            data[tid] = {"VERSION": {
                "ACT": {"AID": "ACT", "MEMLOC": "ACT", "VER": "01-01-1", "PARAM1": ",ACTDAT=", "PARAM2": ",ACTTM=",
                        "ACTDAT": "21-10-01", "ACTTM": "13-38-27"},
                "STBY": {"AID": "STBY", "MEMLOC": "STBY", "VER": "01-01-1", "PARAM1": "", "PARAM2": "", "ACTDAT": "",
                         "ACTTM": ""}
            }}

        # Update only the version information based on GISSUE
        if "GISSUE" in params:
            data[tid]["VERSION"]["ACT"]["VER"] = params["GISSUE"]
            data[tid]["VERSION"]["STBY"]["VER"] = params["GISSUE"]
            print("after update", data)
            print("Updated version for TID {}: {}".format(tid, params["GISSUE"]))
            return (True, "Version updated successfully.")
        else:
            return (False, "Error: GISSUE parameter not found.")



    def handle_rtrv_modifier_new(self, cmd, data, cmd_config):
        tid = cmd["TID"]
        aid = cmd.get("AID", "ACT")
        ret = []
        print("Debug: TID={}, AID={}".format(tid, aid))

        if tid in data and "VERSION" in data[tid]:
            if aid == "ALL":
                for key, version_info in data[tid]["VERSION"].items():
                    response = self.format_response_dict(key, version_info)
                    ret.append(response)
                return (True, ret) if ret else (False, "No version information available")
            elif aid in data[tid]["VERSION"]:
                version_info = data[tid]["VERSION"][aid]
                response = self.format_response_dict(aid, version_info)
                ret.append(response)
                return (True, ret)
            else:
                print("Error: Version information for AID {} not found".format(aid))  # Debug print
                return (False, "Version information for AID {} not found".format(aid))
        else:
            # Handle case where no version information is set by returning a default version
            print("No version information available for TID {}. Using default version.".format(tid))
            default_version_info = {'VER': '01-01-1', 'ACTDAT': '', 'ACTTM': ''}
            response = self.format_response_dict(aid, default_version_info)
            ret.append(response)
            return (True, ret)

    def format_response_dict(self, aid, version_info):
        response = {
            'AID': aid,
            'MEMLOC': aid,
            'VER': version_info.get('VER', 'unknown version'),
            'PARAM1': ',ACTDAT=',
            'PARAM2': ',ACTTM=',
            'ACTDAT': version_info.get('ACTDAT', 'unknown date'),
            'ACTTM': version_info.get('ACTTM', 'unknown time')
        }
        return response


    def handle_login1(self, cmd, data, cmd_config):
        if NECFG[self.GTID].get("authUser") and cmd.get("USER") != data["data"]["user_id"]:
            return (False, None)
        elif NECFG[self.GTID].get("authPasswd") and cmd.get("PASSWD", "") != data["data"]["passwd"]:
            return (False, None)
        ...
        print("Login successful, starting non-firmware alarms")
        #self.start_automsg_thread(data)

        return (True, None)

    def handle_init_sys2(self, cmd, data, cmd_config):
        tid = cmd["TID"]
        params = dict(param.split('=') for param in cmd["KWBLOCK"].split(','))
        ...
        if tid not in data:
            data[tid] = {
                "VERSION": {
                    "ACT": {"AID": "ACT", "MEMLOC": "ACT", "VER": "01-01-1", "PARAM1": ",ACTDAT=", "PARAM2": ",ACTTM=",
                            "ACTDAT": "21-10-01", "ACTTM": "13-38-27"},
                    "STBY": {"AID": "STBY", "MEMLOC": "STBY", "VER": "01-01-1", "PARAM1": "", "PARAM2": "",
                             "ACTDAT": "", "ACTTM": ""}
                },
                "FIRMWARE_ALARMS_ENABLED": False
            }
        ...
        if "GISSUE" in params:
            ...
            print("Starting firmware and non-firmware alarms")
            self.start_automsg_thread(data)

            return (True, "INIT-SYS completed successfully")
        else:
            return (False, "Error: GISSUE parameter not found")

    def start_automsg_thread(self, data):
        thread = threading.Thread(target=self.handle_start_automsg, args=(5, data))
        thread.start()

    def handle_generic1(self, cmd, data, cmd_config):
        '''
        handle_generic -
        "CMD:{{TID}}:{{AID}}:{{CTAG}}:"
        No Action return True and None
        '''
        # Exclude the AID from alarms if it's an INIT-EQPT command
        if cmd["command"] == "INIT-EQPT":
            if "excluded_aids" not in data:
                data["excluded_aids"] = set()
            data["excluded_aids"].add(cmd["AID"])
            print("AID added to excluded alarms:", cmd["AID"])

        if cmd.get("auto_msg"):
            async_msg = threading.Thread(target=self.handle_start_automsg, args=(5, data))
            async_msg.start()
        ...
        if self.AUTOMSG and len(self.LNE) == 1:
            with self.state:
                self.pause = False  # enable autonomous msg
                self.state.notify()  # unblock pause thread
